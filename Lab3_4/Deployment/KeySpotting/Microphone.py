...
class Mic:
	...
	def Record(self,folder,file,i):
		...
		waveFile = wave.open(folder+self.tmp,'wb')
		...
		self.Resample(folder,file_name)

	def Resample(self,file,file_name):
		self.resampler.Resample(folder+self.tmp,file_name)
		os.remove(folder+self.tmp)
...
if __name__=='__main__':
	...
	for i in range(1):
		#Start recording
		file_name = f'silence_{i}.wav'
		microphone.Record(folder,file_name,i)
	#Close Stream
